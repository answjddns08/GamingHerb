...
import {
	addMouseEvent,
	removeMouseEvent,
} from "../composables/useRaycasting.js";
...
function selectTargetForEnemy(skill, enemy) {
	...
	if (skill.type === "heal") {
		const allies = getAliveCharacters(gameManager.enemy).filter(
...
function buildBattleResult() {
	...
	const characters = [...gameManager.friendly, ...gameManager.enemy].map(
		(character) => ({
			name: character.name,
	...
	try {
		...
		for (let i = 0; i < CHARACTER_STATS.orcs.length; i++) {
			...
			if (i < 2) {
				...
				character.addSkill(
					i === 0 ? createSkills.orcRage() : createSkills.orcDefensiveStance(),
				);
			} else {
				// 보스 오크
				character.addSkill(createSkills.orcBasicAttack());
...
<template>
	...
	<div class="UI">
		...
		<SkillCards
			...
			:isFriendly="
				isTargeting ? pendingActor?.isFriendly : selectedCharacter.isFriendly
			"
			:is-progressing="isExecutingTurn"
			@unfocus="
				focusOnTarget(
					new Vector3(0, 0, 0),
					CAMERA_CONFIG.initialCameraPos,
					false,
				)
			"
