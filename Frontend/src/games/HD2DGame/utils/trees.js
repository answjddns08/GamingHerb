...
export const createTree = async (sizeKey, variantIndex, scale, treeConfigs) => {
  ...
  const offset = new Vector2(config.offsets[variantIndex].x, config.offsets[variantIndex].y);
  ...
  return new Promise((resolve, reject) => {
    ...
    loader.load(
      config.path,
      (texture) => {
        const tree = new Tree(texture, config.width, config.height, offset, scale);
        resolve(tree);
      },
      undefined,
      (error) => {
