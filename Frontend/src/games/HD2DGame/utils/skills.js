...
const performMeleeAttackSequence = async (user, target, options) => {
  ...
  const approachPos = targetPos.clone().addScaledVector(direction, -approachDistance);
...
export class Skill {
   ...
   */
  constructor(name, image = "", type, power, coolDown, animationName, description, effect) {
    this.name = name;
    this.image = image;
    this.type = type;
    this.power = power;
    ...
    new Skill(
      ...
      (user, target) => {
        ...
        performMeleeAttackSequence(user, target, {
          ...
          returnDelay: 300,
          onHit: (actualDamage) => {
            console.log(`${user.name}이(가) ${target.name}에게 ${actualDamage} 데미지!`);
          },
        });
      },
  ...
  soldierHeal: (scene) =>
    new Skill(
      ...
      (user, target) => {
        ...
        console.log(`${user.name}이(가) ${target.name}을(를) ${actualHeal} 회복!`);
      },
    ),
    ...
    new Skill(
      ...
      (user, target) => {
        ...
        performMeleeAttackSequence(user, target, {
          ...
          returnDelay: 300,
          onHit: (actualDamage) => {
            console.log(`${user.name}이(가) 강타로 ${target.name}에게 ${actualDamage} 데미지!`);
          },
        });
      },
    ),
  ...
  orcRage: () =>
    new Skill(
      ...
      (user, target) => {
        ...
        console.log(`${target.name}이(가) 분노 상태! ${duration}턴 동안 공격력 +${buffAmount}`);
      },
    ),
  ...
  orcDefensiveStance: () =>
    new Skill(
      ...
      (user, target) => {
        ...
        console.log(`${target.name}이(가) 방어 태세! ${duration}턴 동안 방어력 +${buffAmount}`);
      },
    ),
};
