 ...
 */
export function useTargeting({ registerAction, focusOnTarget, unfocus, selectedCharacter }) {
  ...
  function handleTargetSelection(character) {
    ...
    if (pendingTarget.value === null || pendingTarget.value.name !== character.name) {
    ...
    if (pendingActor.value && pendingSkill.value && pendingTarget.value) {
      registerAction(pendingActor.value, pendingSkill.value, pendingTarget.value);
    }
